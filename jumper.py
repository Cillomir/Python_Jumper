...
screen_y = pygame.display.Info().current_h #1020
...
class Button():
    ...
    def draw(self, pos_x, pos_y, click=False):
...
def backdrop_draw():
    ...
    if player1.lives >= 1: screen.blit(player1.image_mini, (485, 40))
    if player1.lives >= 2: screen.blit(player1.image_mini, (510, 40))
    if player1.lives >= 3: screen.blit(player1.image_mini, (535, 40))
    if Players == 2:
        Print(2, "Player2 Score: " + str(P2_Score), Black, 50, 105)
        Print(2, "Lives: ", Black, 375, 105)
        if player2.lives >= 1: screen.blit(player2.image_mini, (485, 100))
        if player2.lives >= 2: screen.blit(player2.image_mini, (510, 100))
        if player2.lives >= 3: screen.blit(player2.image_mini, (535, 100))
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.left <= 20: self.rect.left = 25
        if self.rect.right >= screen_x - 20: self.rect.right = screen_x - 25
        if self.movex > 0: self.image = self.image_r
        elif self.movex < 0: self.image = self.image_l
...
class Goober(pygame.sprite.Sprite):
    def __init__(self, image, speed):
        ...
        self.speed = speed
        self.movey = 0
        if self.speed == 11: self.rect.y = 500
        self.Dead = False
    ...
    def update(self):
        ...
        if self.Dead == True:
            ...
            self.dead -= 1
            if self.dead % 6 >= 3: self.image = pygame.image.load("Sprites/GooberCrushed1.png").convert_alpha()
            if self.dead % 6 <= 2: self.image = pygame.image.load("Sprites/GooberCrushed2.png").convert_alpha()
            self.rect.y += 25
            if self.dead > 0 and self.speed == 6: Print(2, "+1", Red2, self.rect.x + 5, self.rect.y - 100)
            if self.dead > 0 and self.speed == 8: Print(2, "+1", Red2, self.rect.x + 5, self.rect.y - 100)
            if self.dead > 0 and self.speed == 9: Print(2, "+2", Red2, self.rect.x + 5, self.rect.y - 100)
            if self.dead > 0 and self.speed == 10: Print(2, "+2", Red2, self.rect.x + 5, self.rect.y - 100)
            if self.dead > 0 and self.speed == 12: Print(2, "+3", Red2, self.rect.x + 5, self.rect.y - 100)
            if self.dead > 0 and self.speed == 11: Print(2, "+3", Red2, self.rect.x + 5, self.rect.y - 100)
            if self.dead == 0: pygame.sprite.Sprite.kill(self)
        # ****---***---**--- CHECK FOR A PLAYER COLLISION ---**---***---****
        if pygame.sprite.collide_rect(self, player1) == True and self.Dead == False:
            if player1.movey > 0 and player1.grounded == False:
                if self.speed == 6: P1_Score += 1
                if self.speed == 8: P1_Score += 1
                if self.speed == 9: P1_Score += 2
                if self.speed == 10: P1_Score += 2
                if self.speed == 12: P1_Score += 3
                if self.speed == 11: P1_Score += 3
                self.Dead = True
                player1.rect.y -= 5
            else: 
                player1.lives -= 1
                ...
                if Players == 1:
                    ...
                    for x in all_goobers:
                        x.kill()
                    return
        if pygame.sprite.collide_rect(self, player2) == True and self.dead == False:
            if player2.movey > 0 and player2.grounded == False:
                if self.speed == 6: P2_Score += 1
                if self.speed == 8: P2_Score += 1
                if self.speed == 9: P2_Score += 2
                if self.speed == 10: P2_Score += 2
                if self.speed == 12: P2_Score += 3
                if self.speed == 11: P2_Score += 3
                self.Dead = True
                player2.rect.y -= 5
            else: 
                player2.lives -= 1
        ...
        for g in ground_land:
            ...
            # Landing atop a platform
                ...
                if self.speed == 12 and self.movey >= 0 and random.randrange(0, 8) == 1:
                    self.movey = -45
        if self.speed == 11 and self.rect.y <= 400: self.movey += 5
        if self.speed == 11 and self.rect.y >= 575: self.movey -= 15
...
def Make_Goober():
    ...
    if (P1_Score + P2_Score) <= 10:
        if Passing >= 35 and spawn >= 80: Goober(G_Brown, 6)
    # Score under 15
    elif 10 < (P1_Score + P2_Score) <= 15:
        if Passing >= 35 and spawn >= 85: Goober(G_Brown, 6)
        elif Passing >= 40 and spawn >= 75: Goober(G_Green, 8)
    # Score under 20
    elif 15 < (P1_Score + P2_Score) <= 20:
        if Passing >= 35 and spawn >= 86: Goober(G_Brown, 6)
        elif Passing >= 40 and spawn >= 78: Goober(G_Green, 8)
        elif Passing >= 45 and spawn >= 65: Goober(G_Blue, 9)
    # Score under 25
    elif 20 < (P1_Score + P2_Score) <= 25:
        if Passing >= 32 and spawn >= 90: Goober(G_Brown, 6)
        elif Passing >= 35 and spawn >= 80: Goober(G_Green, 8)
        elif Passing >= 40 and spawn >= 72: Goober(G_Blue, 9)
        elif Passing >= 45 and spawn >= 60: Goober(G_Red, 10)
    # Score under 30
    elif 25 < (P1_Score + P2_Score) <= 30:
        if Passing >= 32 and spawn >= 92: Goober(G_Brown, 6)
        elif Passing >= 36 and spawn >= 84: Goober(G_Green, 8)
        elif Passing >= 40 and spawn >= 78: Goober(G_Blue, 9)
        elif Passing >= 44 and spawn >= 70: Goober(G_Red, 10)
        elif Passing >= 48 and spawn >= 60: Goober(G_Black, 12)
    # Score under 35
    elif 30 < (P1_Score + P2_Score) <= 35:
        if Passing >= 32 and spawn >= 92: Goober(G_Brown, 6)
        elif Passing >= 34 and spawn >= 84: Goober(G_Green, 8)
        elif Passing >= 36 and spawn >= 78: Goober(G_Blue, 9)
        elif Passing >= 38 and spawn >= 70: Goober(G_Red, 10)
        elif Passing >= 40 and spawn >= 60: Goober(G_Black, 12)
    # Score under 40
    elif 35 < (P1_Score + P2_Score) <= 40:
        if Passing >= 30 and spawn >= 95: Goober(G_Brown, 6)
        elif Passing >= 33 and spawn >= 88: Goober(G_Green, 8)
        elif Passing >= 36 and spawn >= 82: Goober(G_Blue, 9)
        elif Passing >= 39 and spawn >= 75: Goober(G_Red, 10)
        elif Passing >= 42 and spawn >= 65: Goober(G_Black, 12)
        elif Passing >= 46 and spawn >= 60: Goober(G_White, 11)
    # Score over 40
    elif (P1_Score + P2_Score) > 40:
        if Passing >= 28 and spawn >= 96: Goober(G_Brown, 6)
        elif Passing >= 30 and spawn >= 92: Goober(G_Green, 8)
        elif Passing >= 32 and spawn >= 88: Goober(G_Blue, 9)
        elif Passing >= 34 and spawn >= 80: Goober(G_Red, 10)
        elif Passing >= 37 and spawn >= 72: Goober(G_Black, 12)
        elif Passing >= 40 and spawn >= 64: Goober(G_White, 11)
...
# *****----*****----*****--- START MENU COMMANDS ---*****----*****----*****
        if start_menu_on == True:
            if event.type == MOUSEBUTTONDOWN:
                ...
                if mouse_click == True and start_true.click == True:
                    ...
                    if Players == 2: player2.rect.x = 50
                    else: player2.kill()
                    start_menu_on = False
                    main_game_on = True
                    Ground_Generate()
                ...
                if mouse_click == True and P1_color_left.click == True: # 0 to 5
                    ...
                    if Players == 2:
                        if P1_Select > 0 and P1_Select != P2_Select + 1: P1_Select -= 1
                        elif P1_Select > 1 and P1_Select == P2_Select + 1: P1_Select -= 2
                ...
                if mouse_click == True and P1_color_right.click == True:
                    ...
                    if Players == 2:
                        if P1_Select < P_Options and P1_Select != P2_Select - 1: P1_Select += 1
                        elif P1_Select < P_Options - 1 and P1_Select == P2_Select - 1: P1_Select += 2
                        elif P1_Select == P_Options and P2_Select != 0: P1_Select = 0
                        elif P1_Select == P_Options and P2_Select == 0: P1_Select = 1
        ...
        if main_game_on == True:
            ...
            if event.type == KEYDOWN:
                    ...
                    if event.key == K_RIGHT:
                        player1.movex = walk
                        player1.face = "right"
                    if event.key == K_DOWN: player1.movex = 0
                    if event.key == K_UP and player1.grounded == True: 
                        player1.movey -= jump
                    ...
                    if event.key == K_d:
                        player2.movex = walk
                        player2.face = "right"
                    if event.key == K_s: player2.movex = 0
                    if event.key == K_w and player2.grounded == True:
                        player2.movey -= jump
    ...
    if main_game_on == True:
        ...
        if Players == 1:
            if P1_Score > 5: Passing += 1
            if P1_Score > 15: Passing += 1
            if P1_Score > 25: Passing += 1
            if P1_Score > 35: Passing += 1
            if P1_Score > 45: Passing += 1
        if Players == 2:
            if P1_Score + P2_Score > 6: Passing += 1
            if P1_Score + P2_Score > 18: Passing += 1
            if P1_Score + P2_Score > 30: Passing += 1
            if P1_Score + P2_Score > 42: Passing += 1
            if P1_Score + P2_Score > 54: Passing += 1
        Passing += 1
        Make_Goober()
        all_players.update()
        all_goobers.update()
